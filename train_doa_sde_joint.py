...
def test_epoch(data_generator, model, activity_loss, criterion, metric_cls, params, device, max_doas):
    ...
    with torch.no_grad():
        for data, target in data_generator.generate():
            ...
            dist_diff = torch.abs(torch.sub(output_norm, target_norm))
...
def train_epoch(data_generator, optimizer, model, activity_loss, criterion, params, device):
    nb_train_batches, train_loss = 0, 0.
    model.train()
    for data, target in data_generator.generate():
        target_activity = target[:, :, -params['unique_classes']:].reshape(-1, params['unique_classes'])
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        if not params['use_two_input']:
            model = models.CRNN(data_in, data_out, params).to(device)
        else:
            if params['rot_cnn']:
                model = models.CRNN_Two(data_in, data_out, params).to(device)
            else:
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            train_loss = train_epoch(data_gen_train, optimizer, model, activity_loss, criterion, params, device)
            ...
            val_metric, val_loss = test_epoch(data_gen_val, model, activity_loss, criterion, val_metric, params, device, params['unique_classes'])
            ...
            print(
                ...
                'DOA error/Distance error/Distance accuracy: {:0.3f}/{}, '
        ...
        test_metric, test_loss = test_epoch(data_gen_test, model, activity_loss, criterion, test_metric, params, device, params['unique_classes'])

        test_hung_loss, test_dist_err, test_dist_acc = test_metric.get_results()

        print(
            'test_loss: {:0.2f}, DOA error/Distance error/Distance accuracy: {:0.3f}/{}'.format(
                test_loss,
